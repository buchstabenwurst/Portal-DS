...
clean:
	@echo "  CLEAN"
	$(V)$(RM) $(ROM) $(DUMP) build $(SDIMAGE) $(SOURCEDIRS)/assets.h $(SOURCEDIRS)/assetsArray.h
...
# compile textures into asstes.h file
$(SOURCEDIRS)/assets.h : $(SOURCEDIRS)/assetsTemplate.h
	$(shell echo -e '/*\n------\nDo not edit this file, it is automatically generated from assetsTemplate.h and grit textures\n------\n*/' > $(SOURCEDIRS)/assets.h)
	$(shell cat $(SOURCEDIRS)/assetsTemplate.h >> $(SOURCEDIRS)/assets.h)
	@echo "  created assets.h"

$(SOURCEDIRS)/assetsArray.h :
	$(shell echo -e '#ifndef ASSETS_ARRAY_H\n#define ASSETS_ARRAY_H' > $(SOURCEDIRS)/assetsArray.h)
	$(shell echo -e '/*\n------\nDo not edit this file, it is automatically generated\n------\n*/' >> $(SOURCEDIRS)/assetsArray.h)
	$(shell echo -e '#include "assets.h"\n\nMaterialMetadata textures[]={\n};\nNE_Material** neMaterials[]={\n};\nNE_Palette** nePallettes[]={\n};\nbool alreadyLoadedTextures[]={\n};' >> $(SOURCEDIRS)/assetsArray.h)
	$(shell echo -e '#endif' >> $(SOURCEDIRS)/assetsArray.h)
	@echo "  created assetsArray.h"

$(BUILDDIR)/%.png.o $(BUILDDIR)/%.h : %.png %.grit $(SOURCEDIRS)/assets.h $(SOURCEDIRS)/assetsArray.h
	@echo "  GRIT    $<"
	@$(MKDIR) -p $(@D)
	$(V)$(BLOCKSDS)/tools/grit/grit $< -ftc -W1 -o$(BUILDDIR)/$*
	$(V)$(CC) $(CFLAGS) -MMD -MP -c -o $(BUILDDIR)/$*.png.o $(BUILDDIR)/$*.c
	$(V)touch $(BUILDDIR)/$*.png.o $(BUILDDIR)/$*.h
	$(shell touch -r $(SOURCEDIRS)/assets.h $(BUILDDIR)/.timestamp)
	$(shell touch -r $(SOURCEDIRS)/assetsArray.h $(BUILDDIR)/.timestamp2)
	$(shell $(GFXDIRS)/../assets.h-helper-script.sh $* $*.grit $(GFXDIRS) $(SOURCEDIRS))
	$(shell touch -r $(BUILDDIR)/.timestamp $(SOURCEDIRS)/assets.h)
	$(shell touch -r $(BUILDDIR)/.timestamp2 $(SOURCEDIRS)/assetsArray.h)
	$(shell rm $(BUILDDIR)/.timestamp)
	$(shell rm $(BUILDDIR)/.timestamp2)

$(BUILDDIR)/%.png.o $(BUILDDIR)/%.h : %.png $(SOURCEDIRS)/assets.h $(SOURCEDIRS)/assetsArray.h
	@echo "  GRIT    $<"
	@$(MKDIR) -p $(@D)
	$(V)$(BLOCKSDS)/tools/grit/grit $< $(shell grep -v '^#' $(GFXDIRS)/default.grit) -ftc -W1 -o$(BUILDDIR)/$*
	$(V)$(CC) $(CFLAGS) -MMD -MP -c -o $(BUILDDIR)/$*.png.o $(BUILDDIR)/$*.c
	$(V)touch $(BUILDDIR)/$*.png.o $(BUILDDIR)/$*.h
	$(shell touch -r $(SOURCEDIRS)/assets.h $(BUILDDIR)/.timestamp)
	$(shell touch -r $(SOURCEDIRS)/assetsArray.h $(BUILDDIR)/.timestamp2)
	$(shell $(GFXDIRS)/../assets.h-helper-script.sh $* $(GFXDIRS)/default.grit $(GFXDIRS) $(SOURCEDIRS))
	$(shell touch -r $(BUILDDIR)/.timestamp $(SOURCEDIRS)/assets.h)
	$(shell touch -r $(BUILDDIR)/.timestamp2 $(SOURCEDIRS)/assetsArray.h)
	$(shell rm $(BUILDDIR)/.timestamp)
	$(shell rm $(BUILDDIR)/.timestamp2)
